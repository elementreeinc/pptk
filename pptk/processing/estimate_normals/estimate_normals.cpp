...
void estimate_normals(vector<T> *eigenvectors, vector<T> *eigenvalues,
  ...
  if (eigenvectors)
    eigenvectors->resize(num_normals * num_eigen * 3);
  if (eigenvalues)
    eigenvalues->resize(num_normals * num_eigen);
  if (neighborhood_sizes)
    neighborhood_sizes->resize(num_normals);
...
template <typename T> struct NumpyTypeNumber {};

template <> struct NumpyTypeNumber<float> {
  static const int value = NPY_FLOAT32;
};

template <> struct NumpyTypeNumber<double> {
...
static PyObject *estimate_normals_wrapper(PyObject *self, PyObject *args,
  ...
  int verbose = 1;
  int num_procs = -1;
  static char *keywords[] = {"points",
                             "k",
                             "r",
                             "subsample",
                             "output_eigenvalues",
                             "output_all_eigenvectors",
                             "output_neighborhood_sizes",
                             "verbose",
                             "num_procs",
                             NULL};
  if (!PyArg_ParseTupleAndKeywords(
          args, kwargs, "Oif|Oiiiii", keywords, &p, &k, &r, &subsample,
          &output_eigenvalues, &output_all_eigenvectors,
          &output_neighborhood_sizes, &verbose, &num_procs)) {
  ...
  if (arr.type_num == NPY_FLOAT32) {
    estimate_normals<float>(
        out1, out2, out3, arr, k, r, ptr_subsample_indices,
        (bool)output_eigenvalues, (bool)output_all_eigenvectors,
        (bool)output_neighborhood_sizes, (bool)verbose, num_procs);
  } else if (arr.type_num == NPY_FLOAT64) {
    estimate_normals<double>(
        out1, out2, out3, arr, k, r, ptr_subsample_indices,
        (bool)output_eigenvalues, (bool)output_all_eigenvectors,
        (bool)output_neighborhood_sizes, (bool)verbose, num_procs);
  } else {
